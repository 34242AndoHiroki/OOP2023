...
namespace Section03 {
    ...
    class Program {
        ...
        private static void NormalSearch() {
            ...
            //Match match = Regex.Match( text , @"\p{IsKatakana}+" );         //カタカナ        \p 単体はなし
            Match match = Regex.Match( text , @"\p{IsHiragana}+" );         //ひらがな      結果：８　の
        ...
        public static void SearchAll() {
            ...
            foreach ( Match match in matches ) {        // var だと参照できない、object型になる
        ...
        public static void SearchAllWithNextMatch() {
            ...
            while ( match.Success ) {       //無くなるまで出力し続ける
                Console.WriteLine( "Index = {0} , Length = {1} , Value = {2}" ,
                                  match.Index , match.Length , match.Value );
        ...
        public static void SearchAndLinq() {
            ...
            var text = "private List<string> results = new List<string>();";
            var matches = Regex.Matches( text , @"\b[a-z]+\b" )         //これだけだと Linq 使えない      \b：ボーダー　
                                .Cast< Match >()                        // IEnumerable< Match >型にキャスト
                                .OrderBy( x => x.Length );              // Linq 使える

            // \b が無いとおかしくなる        ist で引っかかるようになる
